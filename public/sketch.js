...
var app = {
	...
	initialize: (()=> {
		...
		$("#submit").click(function () {
			...
			var songTitle = $("#query").val();
			console.log(songTitle);
			//Execute the Wikipedia API call with the 'newSearchTerm' string as its argument 
			app.trackID(app.URL, songTitle);
		});
	}),

	trackID: function (URL,songTitle) {
		$.ajax({
			url: this.URL + "track.search",
			type: 'GET',
			data: {apikey:'7cb6a00bb49928c2592af2ad44aa7f58',
				  q_track: songTitle,
				  page_size:'1'	
				},
			//dataType: 'jsonp',
...
// 				// debugger;
				...
				trackID = jsonData.message.body.track_list[0].track.track_id;
				console.log(trackID);

				$.ajax({
					url: URL + "track.lyrics.get",
					type: 'GET',
					data: {apikey:'7cb6a00bb49928c2592af2ad44aa7f58',
				    track_id: trackID,		
					},
					...
					success: function(data){
                        ...
                        .map(
                        	...
                        	if(badwords.includes(x)) 
                        		return replacementWord[Math.floor(Math.random()*2)];
                        	else return x;
                        }).join(' ');

                        console.log(words);	
                        $('#right').empty().html(lyrics);
                        $('#left').empty().html(words);
